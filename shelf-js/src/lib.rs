...
//         let uid = uid.unwrap_or_else(|| rand::thread_rng().gen());
...
impl Shelf {
    #[wasm_bindgen(constructor)]
    pub fn new(content: JsValue, client_id: usize) -> Shelf {
        let inner = if content.is_undefined() {
            ShelfCRDT::new(ShelfContent::ShelfMap(HashMap::default()), client_id)
        } else {
            let values = content.into_serde().unwrap_throw();
            ShelfCRDT::from_json_values(values, client_id).unwrap_throw()
        };
        Self(inner)
    }
    ...
    pub fn set(&mut self, path: Array, contents: JsValue, client_id: usize) {
        ...
        if let Some(shelf) = shelf {
            ...
            let content = ShelfContent::from_json_values(json, client_id).unwrap_throw();
            let key = path[last_idx].clone();
            shelf.set(key, content, client_id);
        }
    }
